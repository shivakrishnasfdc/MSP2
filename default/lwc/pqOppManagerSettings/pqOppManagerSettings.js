...
import isCreateOpportunityPlanEnabled from '@salesforce/apex/OpportunityPlanSettingsController.isCreateOpportunityPlanEnabled';
import handleScheduleOpportunityPlanCreation from '@salesforce/apex/OpportunityPlanSettingsController.handleScheduleOpportunityPlanCreation';
import updateCreateOpportunityPlanEnabled from '@salesforce/apex/OpportunityPlanSettingsController.updateCreateOpportunityPlanEnabled';
...
export default class PqOppManagerSettings extends LightningElement {
    ...
    errorMessage = null;
    isGeneral = true;
    isRelationshipMap = false;
    isScheduleJobs = false;
    isBatchStarted = false;
    gentabclass='slds-vertical-tabs__nav-item slds-is-active';
    rmtabclass='slds-vertical-tabs__nav-item';
    shtabclass='slds-vertical-tabs__nav-item';
    ...
    labels2 = {
        Toggle_Schedule_OpportunityPlan_Creation: 'Opportunity Plan Creation Batch',
        Toggle_AutoGenerate_OpportunityPlan: 'Auto Generate Opportunity Plan',
        Off: 'Off',
        On: 'On',
        daysOfData: Days_Of_Data,
        daysOfDataDescription: Days_Of_Data_Description
    ...
    async connectedCallback() {
        ...
        this.today = new Date();
        this.loaded = false;
        await isCreateOpportunityPlanEnabled()
        .then((result) => {
                if (result) {
                    this.oppPlanCreationEnabled = true;
                } else {
                    this.oppPlanCreationEnabled = false;
                }
            })
            .catch((error) => {
        ...
        await getSetting()
            .then((result) => {
                this.setting = result;
                this.days = this.setting.pqcrush__Days_of_Data_Used_In_Analysis__c ? this.setting.pqcrush__Days_of_Data_Used_In_Analysis__c.toString() : '30';
            })
            .catch((error) => {
                this.showNotification('Error', error.message, 'error');
            });
    ...
    handleGenTabClick(){
        if (!this.isGeneral) {
            this.isGeneral = true;
            this.isRelationshipMap = false;
            this.isScheduleJobs = false;
            this.gentabclass = 'slds-vertical-tabs__nav-item slds-is-active';
            this.rmtabclass = 'slds-vertical-tabs__nav-item';
            this.shtabclass = 'slds-vertical-tabs__nav-item';
        }
    }

    handleRmTabClick(){
        if (!this.isRelationshipMap) {
            this.isRelationshipMap = true;
            this.isGeneral = false;
            this.isScheduleJobs = false;
            this.rmtabclass = 'slds-vertical-tabs__nav-item slds-is-active';
            this.gentabclass = 'slds-vertical-tabs__nav-item';
            this.shtabclass = 'slds-vertical-tabs__nav-item';
        }
    }

    handleShTabClick(){
        if (!this.isScheduleJobs) {
            this.isRelationshipMap = false;
            this.isGeneral = false;
            this.isScheduleJobs = true;
            this.shtabclass = 'slds-vertical-tabs__nav-item slds-is-active';
            this.gentabclass = 'slds-vertical-tabs__nav-item';
            this.rmtabclass = 'slds-vertical-tabs__nav-item';
        }
    }
    ...
    handleAutoGenerateOppPlan(event){
        updateCreateOpportunityPlanEnabled({createOppPlan: event.detail.checked})
        .catch((error) => {
                this.showNotification('Error', error.message, 'error');
            });
    }

    handleScheduleOpportunityPlanCreation(){
        handleScheduleOpportunityPlanCreation()
            .catch((error) => {
                this.showNotification('Error', error.message, 'error');
            });
